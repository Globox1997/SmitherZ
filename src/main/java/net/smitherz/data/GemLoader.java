        ...
        try (FileReader fileReader = new FileReader(file)) {
            ...
            for (Entry<String, JsonElement> entry : data.entrySet()) {
                ...
                for (int i = 0; i < jsonArray.size(); i++) {
                    ...
                    map.put(Registries.ATTRIBUTE.getEntry(Identifier.of(attributejsonObject.get("type").getAsString())).get(),
                            new EntityAttributeModifier(Identifier.of(attributejsonObject.get("type").getAsString()), attributejsonObject.get("modifier").getAsJsonObject().get("amount").getAsFloat(),
                ...
                ItemInit.register(entry.getKey(),
                        new Gem(new Item.Settings(), map, jsonObject.get("link_chance").getAsFloat(), jsonObject.get("break_chance").getAsFloat(), jsonObject.get("unlink_chance").getAsFloat(),
                                jsonObject.has("tag") && jsonObject.get("tag") != null ? TagKey.of(RegistryKeys.ITEM, Identifier.of(jsonObject.get("tag").getAsString())) : null));
            }
